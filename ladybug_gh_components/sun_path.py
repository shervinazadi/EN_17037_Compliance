...
def checkConditionalStatement(annualHourlyData, conditionalStatement):
    ...
    indexList, listInfo = lb_preparation.separateList(
        annualHourlyData, lb_preparation.strToBeFound)
      ...
      listNum = []
       for count, let in enumerate(letters):
        ...
        for statemntPart in statement:
            # a copy to make a meaningful string
            ...
            if statemntPart != 'and' and statemntPart != 'or':
                for num in listNum:
                    ...
                    statemntPart = statemntPart.replace(
                        letters[num], toBeReplacedWith, 20000)
                    statementCopy = statementCopy.replace(
                        letters[num], titleToBeReplacedWith, 20000)
                    if statementCopy.find(letters[num]) != -1:
                        break
...
def readLocation(location):
    solarTimeZonesPos = [0, 15, 30, 45, 60, 75,
    ...
    if _solarOrStandardTime_:
        if int(float(timeZone)) == 0:
            longitude = 0.0
        elif int(float(timeZone)) > 0:
            longitude = solarTimeZonesPos[int(float(timeZone))]
        elif int(float(timeZone)) < 0:
            longitude = -solarTimeZonesPos[-int(float(timeZone))]
...
def getHOYsBasedOnPeriod(analysisPeriod, timeStep, lb_preparation):
    stMonth, stDay, stHour, endMonth, endDay, endHour = lb_preparation.readRunPeriod(
        analysisPeriod, True, False)
...
def colorSun(spheres, colors, lb_visualization):
    ...
    for j, sun in enumerate(spheres):
        for face in range(sun.Faces.Count):
            repeatedColors.append(colors[j])
        sunS.Append(sun)
...
def bakePlease(listInfo, sunsJoined, legendSrfs, legendText, textPt, legendFont, textSize, sunPathCrvs, decimalPlaces, lb_visualization):
    ...
    newLayerIndex, l = lb_visualization.setupLayers(
        dataType, 'LADYBUG', layerName, studyLayerName)
    if bakeIt_ == 1:
        lb_visualization.bakeObjects(newLayerIndex, sunsJoined, legendSrfs, legendText,
                                     textPt, textSize, legendFont, sunPathCrvs, decimalPlaces, True)
    else:
        lb_visualization.bakeObjects(newLayerIndex, sunsJoined, legendSrfs, legendText,
                                     textPt, textSize, legendFont, sunPathCrvs, decimalPlaces, False)
...
def main(latitude, longitude, timeZone, elevation, north, hour, day, month, timeStep, analysisPeriod, centerPt, sunPathScale, sunScale, projection, annualHourlyData, conditionalStatement, legendPar, dailyOrAnnualSunPath, bakeIt):
    if _solarOrStandardTime_:
        solarOrStandardTime = _solarOrStandardTime_
    else:
        solarOrStandardTime = False
    ...
    if sc.sticky.has_key('ladybug_release'):
        ...
        sunSc = lb_preparation.setScale(
            sunScale, conversionFac) * scale * conversionFac * 0.007
        ...
        try:
            ...
            if 60 % timeStep != 0:
                ...
                stepDifference = []
                [stepDifference.append(abs(validTimeStep - timeStep))
                 for validTimeStep in validTimeSteps]
                timeStep = validTimeSteps[stepDifference.index(
                    min(stepDifference))]
                print 'Time-step is set to ' + `timeStep`
        except:
            timeStep = 1
            print 'Time-step is set to 1'
        ...
        if len(analysisPeriod) != 0 and analysisPeriod[0] != None:
            ...
            HOYs, months, days = getHOYsBasedOnPeriod(
                analysisPeriod, timeStep, lb_preparation)
        ...
        if conditionalStatement and len(annualHourlyData) != 0 and annualHourlyData[0] != None:
            ...
            titleStatement, patternList = checkConditionalStatement(
                annualHourlyData, conditionalStatement)
        ...
        if printWarning == True:
            ...
            w = gh.GH_RuntimeMessageLevel.Warning
            ghenv.Component.AddRuntimeMessage(
                w, 'Latitude should be between -90 and 90')

        lb_sunpath.initTheClass(float(latitude), northAngle,
                                cenPt, scale, longitude, timeZone)
        ...
        for HOY in HOYs:
            ...
            if lb_sunpath.solAlt >= 0 and patternList[int(round(lb_preparation.date2Hour(m, d, h)))-1]:
                ...
                sunSphere, sunPoint = lb_visualization.sunSpherePt(
                    sunSc, sunPoint, projection, cenPt, scale)
                # find the hour of the year
                sunUpHours.append(lb_preparation.date2Hour(m, d, h))
                sunPosInfo.append(lb_preparation.hour2Date(
                    lb_preparation.date2Hour(m, d, h)))
        ...
        if annualSunPath != False:
            annualSunPathCrvs = [item.ToNurbsCurve() for i, sublist in enumerate(
                lb_sunpath.drawSunPath(solarOrStandardTime)) for item in sublist if i < 2]
        if dailySunPath:
            dailySunPathCrvs = []
            for HOY in HOYs:
                d, m, h = lb_preparation.hour2Date(HOY, True)
                m += 1
                dailySunPathCrvs.append(
                    lb_sunpath.drawDailyPath(m, d).ToNurbsCurve())
        if annualSunPath or dailySunPath:
            # lb_sunpath.drawBaseLines()
            baseCrvs = [rc.Geometry.Circle(cenPt, 1.08*scale).ToNurbsCurve()]

        sunPathCrvs = []
        if annualSunPathCrvs:
            sunPathCrvs = sunPathCrvs + annualSunPathCrvs
        if dailySunPathCrvs:
            sunPathCrvs = sunPathCrvs + dailySunPathCrvs
        if sunPathCrvs != []:
            lb_visualization.calculateBB(baseCrvs, True)

        # Project the curves, if necessary.
        angleCrvs = []
        if projection == 1 or projection == 2:
            sunPathCrvs = lb_visualization.projectGeo(
                sunPathCrvs, projection, cenPt, scale)
        ...
        if len(annualHourlyData) != 0 and annualHourlyData[0] != None:
            try:
                # moving distance for sky domes
                movingDist = 1.5 * lb_visualization.BoundingBoxPar[1]
            except:
                movingDist = 0
            ...
            if len(legendPs) == 0:
                ...
                pass
            elif len(legendPs) == len(listInfo):
                pass
            else:
            ...
            for i in range(len(listInfo)):
                ...
                selList = []
                modifiedsunPosInfo = []
                [selList.append(float(x))
                 for x in annualHourlyData[indexList[i]+7:indexList[i+1]]]
                if listInfo[i][4] != 'Hourly' or str(listInfo[i][5]) != "(1, 1, 1)" or  str(listInfo[i][6]) != "(12, 31, 24)" or len(selList) !=8760:
                    ...
                    for h, hr in enumerate(sunUpHours):
                       #                        value = selList[int(math.floor(hr))] + (selList[int(math.ceil(hr))] - selList[int(math.floor(hr))])* (hr - math.floor(hr))
                        ...
                        values.append(value)
                        modifiedsunPosInfo.append(
                            sunPosInfo[h] + '\n' + ("%.2f" % value) + ' ' + listInfo[i][3])

                if values != [] and sunPathCrvs != []:
                    # mesh colors
                    colors = lb_visualization.gradientColor(
                        values, lowB, highB, customColors)

                    customHeading = '\n\n\n\nSun-Path Diagram - Latitude: ' + `latitude` + '\n'
                    legendSrfs, legendText, legendTextCrv, textPt, textSize = lb_visualization.createLegend(
                        values, lowB, highB, numSeg, listInfo[i][3], lb_visualization.BoundingBoxPar, legendBasePoint, legendScale, legendFont, legendFontSize, legendBold, decimalPlaces, removeLessThan)

                    # generate legend colors
                    legendColors = lb_visualization.gradientColor(
                        legendText[:-1], lowB, highB, customColors)

                    # color legend surfaces
                    legendSrfs = lb_visualization.colorMesh(
                        legendColors, legendSrfs)
                    ...
                    customHeading = customHeading + 'Hourly Data: ' + \
                        listInfo[i][2] + \
                        ' (' + listInfo[i][3] + ')\n' + listInfo[i][1]

                    if titleStatement:
                        resultStr = ("%.1f" % (len(values)/timeStep)) + ' hours of total ' + ("%.1f" % (SUH/timeStep)) + ' sun up hours' + \
                            '(' + ("%.2f" % ((len(values)/timeStep) /
                                             (SUH/timeStep) * 100)) + '%).'
                        # print resultStr
                        customHeading = customHeading + '\n' + titleStatement + '\n' + resultStr

                    titleTextCurve, titleStr, titlebasePt = lb_visualization.createTitle(
                        [listInfo[i]], lb_visualization.BoundingBoxPar, legendScale, customHeading, True, legendFont, legendFontSize, legendBold)

                    legend = lb_visualization.openLegend(
                    ...
                    ##
                    compassCrvsInit, compassTextPts, compassText = lb_visualization. compassCircle(
                        cenPt, northVector, scale, range(0, 360, 30), 1.5*textSize)
                    if projection == 1 or projection == 2:
                        angleCrvs, angleTextPt, angleText = lb_visualization.angleCircle(
                            cenPt, northVector, scale, projection)
                        altitutdeMeshText = lb_visualization.text2srf(
                            angleText, angleTextPt, 'Verdana', textSize/2, legendBold)
                        altitutdeMeshText = lb_preparation.flattenList(
                            altitutdeMeshText)
                    ...
                    if baseCrvs:
                        compassCrvsInit = compassCrvsInit + baseCrvs
                    numberCrvs = lb_visualization.text2srf(
                        compassText, compassTextPts, 'Times New Romans', textSize/1.5, legendBold)
                    ...
                    if legendScale > 1:
                        movingVector = legendScale * movingVector
                    sunsJoined.Translate(movingVector)
                    allSunsJoined.append(sunsJoined)

                    textPt = movePointList(textPt, movingVector)

                    sunPosDup = []
                    [sunPosDup.append(pt) for pt in sunPositions]
                    allSunPositions.append(
                        movePointList(sunPosDup, movingVector))
                    ...
                    if legendBasePoint == None:
                        ...
                        movedLegendBasePoint = rc.Geometry.Point3d.Add(
                            nlegendBasePoint, movingVector)
                    else:
                        movedLegendBasePoint = rc.Geometry.Point3d.Add(
                            legendBasePoint, movingVector)

                    movedTitlebasePt = rc.Geometry.Point3d.Add(
                        titlebasePt, movingVector)
                    ...
                    allLegend.append(lb_visualization.openLegend(
                        [legendSrfs, [lb_preparation.flattenList(legendTextCrv)]]))
                    ...
                    if bakeIt > 0:
                        bakePlease(listInfo[i], sunsJoined, legendSrfs, legendText, textPt,
                                   legendFont, textSize, crvsTemp, decimalPlaces, lb_visualization)
        ...
        # no hourly data tp overlay
            ...
            textSize = legendScale * 0.5 * \
                lb_visualization.BoundingBoxPar[2]/20
            ...
            numberCrvs = lb_visualization.text2srf(
                compassText, compassTextPts, 'Times New Romans', textSize/1.5, legendBold)
            compassCrvs = compassCrvsInit + \
                lb_preparation.flattenList(numberCrvs)

            legendText.extend(compassText)
            textPt.extend(compassTextPts)

            if bakeIt > 0:
                bakePlease(None, sunsJoined, legendSrfs, legendText, textPt, legendFont, textSize,
                           sunPathCrvs + compassCrvsInit + angleCrvsOnly, decimalPlaces, lb_visualization)
